...
bool Client::end_to_end_PIR_query(QueryParameters *_q_params, bool _verbose)
{
    ...
    if (_verbose)
        _q_params->print_db_specific_parameters();
    ...
    if (_verbose)
    {
        ...
        cout << "--------------------- Communication ------------------------------------------" << endl
             ...
             << "\tData Dependant: " << comm_query / 1000 << " KB (Query) + " << comm_response / 1000 << " KB (Reponse)" << endl
             << endl
             << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << endl;
    }

    if (_q_params->write_path != "")
        _q_params->write_results_to_file();
